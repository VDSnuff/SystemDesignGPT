...
export async function handleHandbookProgressGet(request: Request, repository: HandbookProgressRepository) {
  ...
  try {
    ...
    console.error("handbook_progress.read_failed", { userId: user.id, error });
...
export async function handleHandbookProgressPut(request: Request, repository: HandbookProgressRepository) {
  ...
  const parsed = handbookProgressSchema.safeParse(await requestBody(request));
  ...
  try {
    ...
    console.error("handbook_progress.write_failed", { userId: user.id, error });
